...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($_POST['product_id'])) {
        foreach ($_POST['product_id'] as $i => $pid) {
            ...
            // --- Cập nhật stock ---
            if ($loc_id > 0) {
                $checkStock = $conn->prepare("SELECT quantity FROM stock WHERE product_id = ? AND warehouse_id = ? AND location_id = ?");
                $checkStock->bind_param("iii", $pid, $warehouse_id, $loc_id);
                $checkStock->execute();
                $resStock = $checkStock->get_result();
                if ($resStock->num_rows > 0) {
                    $updateStock = $conn->prepare("UPDATE stock SET quantity = quantity + ? WHERE product_id = ? AND warehouse_id = ? AND location_id = ?");
                    $updateStock->bind_param("diii", $qty, $pid, $warehouse_id, $loc_id);
                    $updateStock->execute();
                    $updateStock->close();
                } else {
                    $insertStock = $conn->prepare("INSERT INTO stock (product_id, warehouse_id, location_id, quantity) VALUES (?, ?, ?, ?)");
...
$locations = $conn->query("
    ...
    FROM warehouse_locations wl
    LEFT JOIN stock s ON wl.id = s.location_id
    WHERE wl.warehouse_id = 2 AND wl.status = 'active'
    GROUP BY wl.id
    HAVING current_stock < wl.max_capacity
");
?>
