...
// ================== AJAX: trả về <option> vị trí còn hàng của 1 sản phẩm thành phẩm ==================
...
// ================== XỬ LÝ LƯU PHIẾU XUẤT THÀNH PHẨM ==================
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $prefix = 'PXTP' . date('Ymd'); // PXTP = Phiếu Xuất Thành Phẩm
    ...
    // Bước 3: lưu chi tiết phiếu & trừ tồn kho
    foreach ($_POST['product_id'] as $i => $pid) {
        $pid = intval($pid);
        $qty = floatval($_POST['quantity'][$i]);
        ...
        $stmt = $conn->prepare("
            INSERT INTO sales_order_items (sales_order_id, product_id, quantity, price, location_id) 
            VALUES (?, ?, ?, ?, ?)
        ");
    ...
    $conn->commit();
    $success_message = "✅ Phiếu xuất kho thành phẩm đã được tạo thành công!";
}
...
<div class="form-container">
    <h2>📤 Phiếu xuất kho thành phẩm</h2>
    ...
    <form method="POST">
        ...
        <h2>📦 Danh sách thành phẩm xuất</h2>
